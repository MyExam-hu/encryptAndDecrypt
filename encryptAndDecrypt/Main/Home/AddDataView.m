...
@property (weak, nonatomic) IBOutlet UITextView *describeTextFile;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    self=[[[NSBundle mainBundle] loadNibNamed:@"AddDataView" owner:self options:nil] lastObject];
    if (self) {
        self.frame=frame;
    }
...
- (IBAction)btnAddClick:(id)sender {
    ...
    if ([[self.titleTextFile.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]] isEqual:@""]) {
        ...
        return;
    }else if ([[self.accountTextFile.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]] isEqual:@""]){
        [clsOtherFun showMessageByHUD:@"Please enter your Account!"];
        return;
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    if (buttonIndex == alertView.cancelButtonIndex && alertView.tag==99) {
        ...
        ocls.EncryptAccount=[AES encrypt:self.accountTextFile.text password:[clsOtherFun getAESPassword]];
        ocls.EncryptPasswork=[AES encrypt:self.passworkTextFile.text password:[clsOtherFun getAESPassword]];
        ocls.Content=[AES encrypt:self.describeTextFile.text password:[clsOtherFun getAESPassword]];
        ocls.UserID=1;
        ocls.Status=true;
        if ([clsUserEncryptInfo insertData:ocls]) {
            [clsOtherFun showMessageByHUD:@"添加成功!"];
            [self dismiss];
